 ...
 * but for now...
 */
import init, { tokenize_wasm, parse_wasm, Mode } from "../build/momoa.js";
import wasm from "../build/momoa_bg.wasm";
await init(await wasm());
console.timeEnd("async init");

export function tokenize(text, { mode = "json" } = {}) {
    return tokenize_wasm(text, mode === "jsonc" ? Mode.Jsonc : Mode.Json);
}

export function parse(text, { mode = "json" } = {}) {
    return parse_wasm(text, mode === "jsonc" ? Mode.Jsonc : Mode.Json);
}
